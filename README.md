...
Open-source clean-room replacement for the closed-source `winmm.dll` / `[third-party patch DLL]` shims from the Battlezone 98 Redux Community Patch.
...
winmm.dll (this project)
  ...
  │
  ├── Loads [third-party patch DLL] (optional, for compatibility during development)
  │
...
All 27+ patches from `[third-party patch DLL]` are reimplemented here as clean-room code:
...
The BZR.exe addresses are stored at runtime in `[third-party patch DLL]`'s data section and
...
Match these to the patch names printed in `[third-party patch].log` and fill in `patches.h`.
