...
//
// Reconstructed from analysis of [third-party patch DLL] ([third-party patch]_hopfix_decompiled.txt,
...
void __declspec(naked) __cdecl Trampoline_HopFix1()
{
    ...
    __asm
    {
        ...
        // Clean-room equivalent of [third-party patch DLL] trampoline block.
        mov  eax, [ebp - 4]          // 8B 45 FC
        mov  [ebp - 4], eax          // 89 45 FC
        mov  ecx, [ebp - 4]          // 8B 4D FC
        // [third-party patch DLL] calls an internal helper here (FUN_1000caf0); omitted in minimal hop-fix mode.
        mov  eax, [ebp - 4]
        mov  ecx, [eax + 0x138]
