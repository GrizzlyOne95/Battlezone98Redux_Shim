...
//
// Reconstructed from: [third-party patch]_hopfix_decompiled.txt (FUN_1000eb30),
// FINAL_RE_REPORT.md, COMPLETE_RE_REPORT.md
//
// The original [third-party patch DLL] patch flow:
//  1. Open [third-party patch].log
...
namespace BZROpenShim
{
    ...
    // Mirrors [third-party patch DLL] logic: reads VS_FIXEDFILEINFO.dwFileVersionMS fields
    ...
    static uint32_t GetBZRVersion()
    {
        ...
        // [third-party patch DLL] reads *(ushort*)((int)local_160 + 0xe) 
    ...
    // Mirrors [third-party patch DLL] FUN_10004930 / FUN_10003810
    ...
    static void ResolvePointers(uint32_t hopFix1Addr, uint32_t hopFix2Addr, uint32_t hopFix3Addr, uint32_t probeMapSortingAddr, uint32_t probeMapFilter1Addr, uint32_t probeMapListFix1Addr, uint32_t probeMapListFix2Addr)
    {
        ...
        // Hop-Fix 1 - [third-party patch DLL] jumps back to patch site + 0xE
        ...
        // Hop-Fix 2 - [third-party patch DLL] jumps back to patch site + 0x13.
        if (hopFix2Addr) {
        ...
        // Hop-Fix 3 - [third-party patch DLL] jumps back to patch site + 7
        // (CALL EDX + PUSH 0x930).
        if (hopFix3Addr) {
    ...
    static void ScanForPatchAddresses(std::vector<PatchDef>& patches)
    {
        ...
        for (auto& p : patches)
        {
            ...
            else if (strcmp(p.name, "[third-party patch] BZRNET Integration HOST") == 0)
            {
            ...
            else if (strcmp(p.name, "[third-party patch] BZRNET Integration CLIENT") == 0)
            {
    ...
    static void FillJmp5Payloads(std::vector<PatchDef>& patches)
    {
        ...
        struct { const char* name; void* fn; } map[] =
        {
            ...
            { "[third-party patch] BZRNET Integration HOST",                   (void*)Trampoline_BzrnetHost },
            { "[third-party patch] BZRNET Integration CLIENT",                 (void*)Trampoline_BzrnetClient },
