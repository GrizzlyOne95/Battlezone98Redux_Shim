...
// Site: 0x00799774 inside FUN_00799770
//
// Stock is a thiscall wrapper:
//   mov [ebp-4], ecx          ; list widget
//   mov eax, [ebp+8]          ; signed wheel/key delta
//   push eax
//   mov ecx, [ebp-4]
//   call FUN_007a3bd0         ; thiscall (ecx=list, stack=delta)
//   return 1
//
// FUN_007a3bd0 steps the list by 120-unit notches via ScrollUp 0x007CB500
// / ScrollDown 0x007CB540, both thiscalls that read [this+0x150].
//
// The previous reconstruction called ScrollUpdateHelper after overwriting
// [ebp-4] with the delta, then invoked ScrollUp as a cdecl with leftover
// ecx. That first-chance AV'd (esi=0) on every wheel tick and the fallback
// used [[ebp]+0x1C8] (a stack frame, not the widget). Replay stock.
// Hop-Fix 1/2/3 still save/restore the selection across list rebuilds.
// -----------------------------------------------------------------------
