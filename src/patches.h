...
// [third-party patch DLL] exposes a Lua module "install" function that is called from
// Lua mission scripts via require("[third-party patch DLL]").install().
//
...
//
// Each JMP hook is:
//   E9 [rel32]   -- relative jump to replacement code inside [third-party patch DLL]
//
...
//
// --- Address notes ---
// The BZR.exe addresses stored in [third-party patch DLL]'s DAT_1002axxx variables are
...
namespace BZROpenShim
{
    ...
    // These mirror the [third-party patch DLL] hop-fix return-address pointers.
    // -----------------------------------------------------------------------
    ...
    inline std::vector<PatchDef> BuildPatchList()
    {
        ...
        return
        {
            ...
            { 0x0, PT::JMP5, {}, "[third-party patch] BZRNET Integration HOST", false },
            { 0x0, PT::JMP5, {}, "[third-party patch] BZRNET Integration CLIENT", false },
