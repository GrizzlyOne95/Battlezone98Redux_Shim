# openshim_wrap.ps1 - wrap the game launch, bundle the session, upload it to Discord.
#
# The Windows counterpart of upload/openshim_wrap.sh. Read that file's header for why
...
# obvious form -- cmd /c "...openshim_wrap.bat" %command% -- has four, and cmd hands
# itself `...openshim_wrap.bat" "...game.exe`, then fails with "The filename, directory
...
# check - the same rule openshim_wrap.sh uses. This was previously
...
# `cmd /c openshim_wrap.bat`, whose console dies with the process, so a launch that
...
$WrapLog = Join-Path $DataDir "openshim_wrap.log"
function Write-WrapLog {
    param([string]$Message)
    $line = "[{0}] {1}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Message
    Write-Host "[openshim_wrap] $Message"
    try {
...
function Get-Conf {
    ...
    if (Test-Path $ConfFile) {
        foreach ($line in (Get-Content $ConfFile)) {
            ...
            if ($line -match '^\s*([A-Za-z_]+)\s*=\s*(.*)$') {
                ...
                switch ($k) {
                    "OPENSHIM_WEBHOOK" { $conf.Webhook = $v }
                    "OPENSHIM_PLAYER" { $conf.Player = $v }
                    "OPENSHIM_INCLUDE_PROTON" { $conf.IncludeProton = ($v -eq "1") }
                    "OPENSHIM_UPLOAD_MENU" { $conf.UploadMenu = ($v -eq "1") }
                    "BZ_WEBHOOK" { if (-not $conf.Webhook) { $conf.Webhook = $v } }
                    "BZ_PLAYER"  { if (-not $conf.Player) { $conf.Player = $v } }
    ...
    if ($env:OPENSHIM_UPLOAD_WEBHOOK) { $conf.Webhook = $env:OPENSHIM_UPLOAD_WEBHOOK }
    elseif ($env:BZ_UPLOAD_WEBHOOK) { $conf.Webhook = $env:BZ_UPLOAD_WEBHOOK }
    return $conf
}
...
function Invoke-Setup {
    ...
    Write-Host ""
    Write-Host "openshim_wrap setup"
    Write-Host "------------------"
    ...
    @(
        "# Written by openshim_wrap.ps1 -Setup. Do not commit this file."
    ...
    Write-Host ""
    Write-Host '  cmd /c ""%LOCALAPPDATA%\openshim\openshim_wrap.bat" %command%"'
    Write-Host ""
    if ((Split-Path -Parent $PSCommandPath) -ne $DataDir) {
        Write-Host "Copy this script and openshim_wrap.bat into $DataDir first."
        Write-Host ""
    }
    Write-Host "What gets sent: openshim.log, openshim_crash.log, the proxy log,"
    Write-Host "BZLogger (before and after), multi.ini, and a meta file."
    Write-Host "Bundles contain every peer's public IP, which is why the destination"
    Write-Host "is a private channel."
    Write-Host ""
    Write-Host "A console window stays open while the game runs - that is this wrapper"
    Write-Host "waiting to bundle on exit. Closing it kills the upload, not the game."
    return 0
}
...
# OpenShim writes logs\name; the stock game writes name in the game root.
# Prefer logs\ when both exist.
...
function Copy-Harvested {
    param([string]$GameDir, [string]$Name, [string]$DestDir, [string]$DestName = "")
    if (-not $DestName) { $DestName = $Name }
    $src = Resolve-HarvestedPath -GameDir $GameDir -Name $Name
    if (-not $src) { return $false }
    Copy-Item -Force -LiteralPath $src -Destination (Join-Path $DestDir $DestName)
    return $true
}

function Get-GamePlayerName([string]$GameDir) {
    if (-not $GameDir) { return $null }
    $log = Resolve-HarvestedPath -GameDir $GameDir -Name "BZLogger.txt"
...
function New-BundleAndUpload {
    param([string]$GameDir, [string]$Snapshot, [string]$ShimSnapshot, [string]$StartUtc, [int]$ExitCode)
    ...
    $bundle = Join-Path $Work "openshim_${player}_${stamp}"
    ...
    # session's log, which this launch has already overwritten. openshim.log
    # is opened with "w" each launch, so it needs the same treatment.
    if ($Snapshot -and (Test-Path $Snapshot)) {
        Copy-Item -Force $Snapshot (Join-Path $bundle "BZLogger.prelaunch.txt")
    }
    if ($ShimSnapshot -and (Test-Path $ShimSnapshot)) {
        Copy-Item -Force $ShimSnapshot (Join-Path $bundle "openshim.prelaunch.log")
    }

    foreach ($f in @("BZLogger.txt", "winmm_proxy.log", "dsound_proxy.log",
                     "multi.ini", "openshim.log", "openshim_crash.log")) {
        Copy-Harvested -GameDir $GameDir -Name $f -DestDir $bundle | Out-Null
    }
    ...
    $sessionFiles = @(
        "bz_buffer_log.bin", "bz_buffer_log.meta.txt", "capture_verify.txt"
    )
    foreach ($f in $sessionFiles) {
        $src = Resolve-HarvestedPath -GameDir $GameDir -Name $f
        if (-not $src) { continue }
        if ((Get-Item -LiteralPath $src).LastWriteTimeUtc -lt $SessionStart) {
...
if (-not $Command -or $Command.Count -eq 0) {
    ...
    Write-Host "Usage: openshim_wrap.ps1 [-Setup|-Status|-Retry] | <game command...>"
    exit 0
}
...
try {
    ...
    # Steam launch line: one KEY=VALUE per line in %APPDATA%\openshim\
    # openshim.env (OPENSHIM_ or BZ_ keys only - this file is a tuning knob, not a general
    ...
    $envFile = Join-Path $ConfDir "openshim.env"
    if (Test-Path -LiteralPath $envFile) {
        foreach ($line in (Get-Content $envFile)) {
            if ($line -match '^\s*((?:OPENSHIM_|BZ_)[A-Za-z0-9_]+)\s*=\s*(.*)$') {
                $k = $Matches[1]; $v = $Matches[2].Trim().Trim("'").Trim('"')
                Set-Item -Path "env:$k" -Value $v
                $SessionOverrides += "$k=$v"
                Write-WrapLog "session override: $k=$v (from openshim.env)"
            }
        }
    }
    ...
    if ($gameDir) {
        $bzPrev = Resolve-HarvestedPath -GameDir $gameDir -Name "BZLogger.txt"
        if ($bzPrev) {
            $snapshot = Join-Path $Work "BZLogger.prelaunch.$PID.txt"
            Copy-Item -Force -LiteralPath $bzPrev $snapshot
            Write-WrapLog "snapshotted the previous session's BZLogger"
        }
        $shimPrev = Resolve-HarvestedPath -GameDir $gameDir -Name "openshim.log"
        if ($shimPrev) {
            $shimSnapshot = Join-Path $Work "openshim.prelaunch.$PID.log"
            Copy-Item -Force -LiteralPath $shimPrev $shimSnapshot
            Write-WrapLog "snapshotted the previous session's openshim.log"
        }
    }
...
try {
    ...
    # openshim via `cmd /c openshim_wrap.bat` -- and the bundle we upload contains
...
if ($gameDir) {
    try {
        New-BundleAndUpload -GameDir $gameDir -Snapshot $snapshot -ShimSnapshot $shimSnapshot -StartUtc $startUtc -ExitCode $rc
    } catch {
...
if ($shimSnapshot -and (Test-Path $shimSnapshot)) { Remove-Item -Force $shimSnapshot }
