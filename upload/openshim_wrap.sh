#!/usr/bin/env bash
# openshim_wrap.sh — wrap the game launch, bundle the session, upload it to Discord.
#
...
#
#   WINEDLLOVERRIDES="winmm=n,b;dsound=n,b" ~/.local/share/openshim/openshim_wrap.sh %command%
#
...
# `openshim_wrap.sh --setup` once; it writes ~/.config/openshim/upload.conf with
# mode 600.
#
# PRIVACY: a bundle contains every peer's public IP. The destination must be a
# private channel. See upload/README.md.
set -uo pipefail
...
if [[ -f "$SELF_DIR/upload.conf" ]]; then
    ...
    OUTBOX="${XDG_DATA_HOME:-$HOME/.local/share}/openshim/outbox"
    WORK="${XDG_DATA_HOME:-$HOME/.local/share}/openshim/work"
fi
...
LOG_FILE="$CONF_DIR/openshim_wrap.log"
LOG_MAX_BYTES=$((256 * 1024))

log() {
    printf '[openshim_wrap] %s\n' "$*" >&2
...
usage() {
    cat <<'EOF'
Usage:
  openshim_wrap.sh --setup            configure the webhook and player name
  openshim_wrap.sh --retry            send anything parked in the outbox and exit
  openshim_wrap.sh --status           show configuration and outbox state
  openshim_wrap.sh %command% [args...] launch the game, then bundle and upload
...
~/.local/share/openshim/ on a native install and at
~/.var/app/com.valvesoftware.Steam/data/openshim/ under Flatpak. Snap
...
~/snap/steam/common/.local/share/openshim/ (the installer puts it there).
...
load_conf() {
    ...
    # bundles are a few kB. OPENSHIM_UPLOAD_MENU=0 in upload.conf restores the skip.
    OPENSHIM_UPLOAD_MENU=1
    BZ_WEBHOOK=""
    BZ_PLAYER=""
    BZ_INCLUDE_PROTON=0
    BZ_UPLOAD_MENU=""
    ...
    [[ -z "$OPENSHIM_WEBHOOK" && -n "${BZ_WEBHOOK:-}" ]] && OPENSHIM_WEBHOOK="$BZ_WEBHOOK"
    [[ -z "$OPENSHIM_PLAYER" && -n "${BZ_PLAYER:-}" ]] && OPENSHIM_PLAYER="$BZ_PLAYER"
    [[ -z "${OPENSHIM_INCLUDE_PROTON:-}" && -n "${BZ_INCLUDE_PROTON:-}" ]] && OPENSHIM_INCLUDE_PROTON="$BZ_INCLUDE_PROTON"
    [[ -n "${BZ_UPLOAD_MENU:-}" ]] && OPENSHIM_UPLOAD_MENU="$BZ_UPLOAD_MENU"
    OPENSHIM_WEBHOOK="${OPENSHIM_UPLOAD_WEBHOOK:-${BZ_UPLOAD_WEBHOOK:-$OPENSHIM_WEBHOOK}}"
}
...
EOF
    ...
    read -r -p "Discord webhook URL${OPENSHIM_WEBHOOK:+ [keep existing]}: " url
    [[ -z "$url" ]] && url="$OPENSHIM_WEBHOOK"
    if [[ -z "$url" ]]; then
    ...
    read -r -p "Your player name (empty = your in-game name, read from BZLogger at upload time)${OPENSHIM_PLAYER:+ [$OPENSHIM_PLAYER]}: " player
    [[ -z "$player" ]] && player="$OPENSHIM_PLAYER"
    ...
    umask 077
    cat >"$CONF_FILE" <<EOF
# Written by openshim_wrap.sh --setup. Do not commit this file.
OPENSHIM_WEBHOOK='$url'
OPENSHIM_PLAYER='$player'
OPENSHIM_INCLUDE_PROTON=$proton
EOF
    chmod 600 "$CONF_FILE"
    ...
    if [[ "$self_real" == "$HOME/snap/steam/common/"* ]]; then
        echo '  WINEDLLOVERRIDES="winmm=n,b;dsound=n,b" "$SNAP_USER_COMMON/.local/share/openshim/openshim_wrap.sh" %command%'
    elif [[ "$self_real" == "$(readlink -f "${XDG_DATA_HOME:-$HOME/.local/share}/openshim/openshim_wrap.sh" 2>/dev/null || true)" ]]; then
        echo '  WINEDLLOVERRIDES="winmm=n,b;dsound=n,b" "${XDG_DATA_HOME:-$HOME/.local/share}/openshim/openshim_wrap.sh" %command%'
    else
        echo "  WINEDLLOVERRIDES=\"winmm=n,b;dsound=n,b\" $self_real %command%"
    fi
    echo
    ...
    echo
    echo "What gets sent: openshim.log, openshim_crash.log, the proxy log,"
    echo "BZLogger (before and after), multi.ini, and a meta file."
    echo "Bundles contain every peer's public IP, which is why the destination"
    echo "is a private channel."
    echo
    echo "Copy the launch line printed above. Don't guess between native/Flatpak"
    echo "and Snap: a wrapper path that does not exist inside the sandbox kills"
    echo "the launch instead of starting the game."
}
...
do_status() {
    ...
    echo "webhook     : $( [[ -n "$OPENSHIM_WEBHOOK" ]] && echo 'configured' || echo 'NOT SET')"
    echo "player      : ${OPENSHIM_PLAYER:-<auto: in-game name, read from BZLogger at upload time>}"
    echo "proton logs : $( [[ "${OPENSHIM_INCLUDE_PROTON:-0}" == "1" ]] && echo included || echo skipped)"
    echo "outbox      : $OUTBOX"
    if [[ -d "$OUTBOX" ]]; then
        local n
...
# OpenShim writes logs/name; the stock game writes name in the game root.
harvested_path() {
    local game_dir="$1" name="$2"
    [[ -n "$game_dir" ]] || return 1
    if [[ -f "$game_dir/logs/$name" ]]; then
        printf '%s' "$game_dir/logs/$name"
        return 0
    fi
    if [[ -f "$game_dir/$name" ]]; then
        printf '%s' "$game_dir/$name"
        return 0
    fi
    return 1
}
...
harvest_copy() {
    local game_dir="$1" name="$2" dest="$3" dest_name="${4:-$2}"
    local src
    src="$(harvested_path "$game_dir" "$name" || true)"
    [[ -n "$src" ]] || return 1
    cp -f "$src" "$dest/$dest_name"
}

game_player_name() {
    local game_dir="$1" log
    log="$(harvested_path "$game_dir" "BZLogger.txt" || true)"
    [[ -n "$log" ]] || return 1
    sed -n 's/.*Authenticated to BZRNet As S[0-9]*://p' "$log" | tail -1
}
    ...
    for arg in "$@"; do
        case "$arg" in
            */steamapps/common/*/*)
                ...
                [[ -d "$dir" ]] || continue
                if [[ -f "$dir/battlezone98redux.exe" || -f "$dir/BZLogger.txt" \
                   || -f "$dir/logs/openshim.log" ]]; then
...
post_file() {
    ...
    if command -v python3 >/dev/null 2>&1; then
        ...
        BZW_URL="$OPENSHIM_WEBHOOK" BZW_FILE="$file" BZW_PAYLOAD="$payload" \
...
try:
    ...
    print('[openshim_wrap] python3 upload failed: %s' % e, file=sys.stderr)
    sys.exit(1)
PYEOF
        rc=$?
...
collect_and_upload() {
    local game_dir="$1" snapshot="$2" shim_snapshot="$3" start_utc="$4" exit_code="$5"
    ...
    bundle="$WORK/openshim_${player}_${stamp}"
    mkdir -p "$bundle"
    ...
    # which game N's launch has already overwritten. openshim.log is opened
    # with "w" each launch, so it needs the same treatment.
    if [[ -f "$snapshot" ]]; then
        cp -f "$snapshot" "$bundle/BZLogger.prelaunch.txt"
    fi
    if [[ -f "$shim_snapshot" ]]; then
        cp -f "$shim_snapshot" "$bundle/openshim.prelaunch.log"
    fi

    local f src
    for f in BZLogger.txt dsound_proxy.log winmm_proxy.log multi.ini \
    ...
    # for OpenShim minidumps: the splitter already handles Discord's 10 MB cap.
    for f in bz_buffer_log.bin bz_buffer_log.meta.txt capture_verify.txt; do
        src="$(harvested_path "$game_dir" "$f" || true)"
        [[ -n "$src" ]] || continue
        if [[ -n "${start_epoch:-}" ]] && \
           (( $(stat -c %Y "$src" 2>/dev/null || echo 0) < start_epoch )); then
    ...
    if [[ -d "$game_dir/logs" ]]; then
        ...
        for dump in "$game_dir/logs"/openshim_crash_*.dmp; do
            [[ -f "$dump" ]] || continue
            if [[ -n "${start_epoch:-}" ]] && \
               (( $(stat -c %Y "$dump" 2>/dev/null || echo 0) < start_epoch )); then
                log "skipping stale $(basename "$dump") (predates this session)"
                continue
            fi
            cp -f "$dump" "$bundle/"
        done
    ...
    if [[ "${OPENSHIM_INCLUDE_PROTON:-0}" == "1" ]]; then
...
# Pre-launch: snapshot BZLogger and openshim.log before the game truncates them.
snapshot=""
shim_snapshot=""
start_utc="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
start_epoch="$(date +%s)"

# Per-session OPENSHIM_* / BZ_* overrides, for switching A/B arms without editing the Steam
# launch line: one KEY=VALUE per line in $CONF_DIR/openshim.env (OPENSHIM_ or BZ_ keys only —
...
# arm it was.
overrides=""
if [[ -f "$CONF_DIR/openshim.env" ]]; then
    while IFS= read -r _line; do
        case "$_line" in
            OPENSHIM_[A-Za-z0-9_]*=*|BZ_[A-Za-z0-9_]*=*)
                _k="${_line%%=*}"
                _v="${_line#*=}"
                _v="${_v%\'}"; _v="${_v#\'}"; _v="${_v%\"}"; _v="${_v#\"}"
                export "${_k}=${_v}"
                overrides="$overrides $_k=$_v"
                log "session override: $_k=$_v (from openshim.env)"
                ;;
        esac
    done <"$CONF_DIR/openshim.env"
fi
if [[ -n "$game_dir" ]]; then
    _bz_prev="$(harvested_path "$game_dir" "BZLogger.txt" || true)"
    if [[ -n "$_bz_prev" ]]; then
        snapshot="$WORK/BZLogger.prelaunch.$$"
        cp -f "$_bz_prev" "$snapshot" 2>/dev/null || snapshot=""
        [[ -n "$snapshot" ]] && log "snapshotted the previous session's BZLogger"
    fi
    _shim_prev="$(harvested_path "$game_dir" "openshim.log" || true)"
    if [[ -n "$_shim_prev" ]]; then
        shim_snapshot="$WORK/openshim.prelaunch.$$"
        cp -f "$_shim_prev" "$shim_snapshot" 2>/dev/null || shim_snapshot=""
        [[ -n "$shim_snapshot" ]] && log "snapshotted the previous session's openshim.log"
    fi
    unset _bz_prev _shim_prev
fi
...
if [[ -n "$game_dir" ]]; then
    collect_and_upload "$game_dir" "$snapshot" "$shim_snapshot" "$start_utc" "$rc" || \
        log "bundle collection failed; the game session itself was unaffected"
fi
[[ -n "$snapshot" ]] && rm -f "$snapshot"
[[ -n "$shim_snapshot" ]] && rm -f "$shim_snapshot"
