...
#### Two texture units, suppression, and in-place conversion (2026-09-26)

On `isdfms10` under DX11, the ISDF Chronicles rain material `xrain` threw `Attempted to render to a D3D11 device without both vertex and fragment shaders` about 7,500 times in 56 seconds of mission time. It has two texture units: unit 0 has `scroll_anim`, and unit 1 is a mask with `colour_op alpha_blend`. The ladder only supported one unit, and declining left Ogre on its fixed-function technique. Each throw also aborted the rest of that frame. That hid two more DX11 faults behind it, and both are fixed here as well.

- **Two stages** (`CompatPath::FixedFuncTextured2`). The runtime now reads each stage's combine state through the exported `TextureUnitState::getColourBlendMode`, `getAlphaBlendMode` and `getTextureCoordSet`. The layout is checked through the `blendType` field. The supported set is `IsSupportedTwoStageCombo`: both stages on UV set 0, both with default alpha, stage 0 `modulate`, and stage 1 `modulate`, `add` or `alpha_blend`. Programs: `OSE_FixedFunc_Textured2_vertex[_novc]`, which gives each stage its own `texture_matrix`, and `OSE_FixedFunc_Textured2_fragment_{modulate,add,alphablend}`. A corpus scan of every `.material` in the install found 23 two-unit fixed-function passes, all in ISDF Chronicles, and all of them are covered. Five are the rain family (`xrain`, `xrainL`, `xrainR`, `acidrain`, `rainbox_BZBase_raintest`) with `alpha_blend`, and 18 are ported BZBase emissive overlays with a default-modulate `EmissiveMap` unit. The single-unit path keeps its historical modulate assumption, so existing content does not change path.
- **Suppression** (`CompatPath::SuppressPass`). This covers a fixed-function pass outside the support set, or any multi-pass fixed-function technique, when every technique Ogre could fall back to still has a shaderless pass. In that case the pass binds `OSE_FixedFunc_Suppress_vertex/fragment`, which reads POSITION only, clips everything and discards. Nothing is drawn and nothing throws. The ladder logs it once as `[DX11COMPAT] material=<m> path=suppress reason=<r> ... action=skip-draw`. When a programmable fallback exists, the historical decline is kept.
- **In-place conversion of sibling techniques.** A fixed-function technique whose scheme matches the active scheme never misses, so it never reaches the ladder. `water` is the example: its only technique sits in `scheme glow`, and the glow compositor drew it shaderless. Once a material has been synthesized on a miss, every other fully shaderless technique it owns is converted in place with the same policy. The log line is `[DX11COMPAT] material=<m> converted-in-place technique=<n> scheme=<s> path=<p> vs=<vs> ps=<ps>`. Passes this layer converted classify as native on later misses.
- **Native input guards** (`Dx11Compat::NativeInputGuard`). The glow compositor draws every non-glowing object with the stock `Glow/Null` material. It uses `Untextured_vertex`, which reads COLOR0, multiplied by a black diffuse. On a mesh without DIFFUSE (`prop.mesh`) that threw `Unable to set D3D11 vertex declaration` on every frame. Once, when both stock program names still match and the pass diffuse is black, the vertex program is rebound to `OSE_FixedFunc_Untextured_vertex_novc`. The output stays black, and the draw binds on any mesh. This is logged as `native-input-guard ... action=applied`, or as `skipped:<why>`.
