...
| P1-2 | **Legacy-1.4 AI branch ships a `suppress = true` placeholder**: legacy craft never leave flee state 9 by timeout; also team-filtered ProximityMine "detonates" by OR-ing removal flags without spawning ordnance, and the magnet twin skips base `Mine::Simulate` when class is null. Either finish or gate off. **Resolved 2026-09-26** (`agent/legacy-ai-placeholder-gate`) by gating off what was unfinished and keeping what works. (1) The D3 placeholder is gone, so legacy flee exits follow stock. The Redux decompile (FUN_00478a50, switch index = state - 2) shows flee leaves for blast on IsStuck *or* the 3 s timeout, so rewriting every 9->10 pinned stuck craft. Finishing it needs the stuck verdict without re-calling FUN_006027f0, which resets its sample; that recipe is in the comment. D1 and D4 still apply, and the first legacy craft logs one line naming which parts are ported. (2) A team-filtered ProximityMine now runs stock `Simulate`, so it explodes again on stock targeting, and one line says the filter is not honoured yet. The stock detonation block (terrain snap, debris, owner, ExplosionClass::Build FUN_004cb7b0 on class+0x170, MP notify) is recorded in the comment for whoever finishes it. (3) A classless MagnetMine `__leave`s the attraction block and still runs base `Mine::Simulate`. The empty sound-handle block is deleted. `IsBuildingStub` was already gone. Validated: solution build (the two C4189 warnings from the dead D3 block are gone), ctest 47/47. No CR or installed Workshop content uses `legacyAI`/`tanklegacy`/`teamFilter`, so a GOG boot would not reach these paths. A behaviour check with a test ODF is **unverified**. | `bzr_hooks.cpp` 22931-22951, 32887-32900, 32679 | E |
