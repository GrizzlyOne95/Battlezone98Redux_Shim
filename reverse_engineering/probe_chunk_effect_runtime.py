        ...
        for entry in sample.get("entries", []):
            ...
            if obj:
                lines.append(
                    ...
                    f"class={obj.get('class_id')} flags={hex32(obj.get('flags') or 0)} "
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if sample["count"] > 0:
                ...
                for entry in sample["entries"][: args.max_objects]:
                    if entry["obj_ptr"]:
                        entry["obj_snapshot"] = snapshot_obj76(handle, entry["obj_ptr"])
